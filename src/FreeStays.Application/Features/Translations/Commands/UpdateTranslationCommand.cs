...
public record UpdateTranslationCommand : IRequest<TranslationDto>
{
...
public class UpdateTranslationCommandHandler : IRequestHandler<UpdateTranslationCommand, TranslationDto>
{
    ...
    public UpdateTranslationCommandHandler(ITranslationRepository translationRepository, IUnitOfWork unitOfWork)
    {
    ...
    {
        var translation = await _translationRepository.GetByKeyAsync(
            request.Locale, 
            request.Namespace, 
            request.Key, 
            cancellationToken);
        ...
        else
        {
            ...
            translation.UpdatedBy = Guid.TryParse(request.UpdatedBy, out var userId2) ? userId2 : null;
